...
void Voice::Start()
{
	...
	if (!pSoundFirst)
		return;
	if (FAILED(pXAudio2->CreateSourceVoice(&pSourceVoice, &pSoundFirst->pSound->wfm, 0, XAUDIO2_DEFAULT_FREQ_RATIO, this, NULL, NULL)))
		return;
...
void Voice::Stream()
{
	...
	//if (state.BuffersQueued < VOICE_BUFFER_COUNT - 1)
	for (int i = state.BuffersQueued; i < VOICE_BUFFER_COUNT; i++)
	{
		OggVorbis_File* vf = pSoundFirst->pSound->vf;
